...
def run_master_scan():
    ...
    print("="*60)
    print(f"[*] Início da varredura: {time.ctime()}")

    all_findings = []

    # 1. Varredura de Processos e Memória Viva (User-Mode)
    ...
    # 2. Varredura de Kernel-Mode e Detecção de Rootkits
    if sys.platform == "win32":
        print("\n[2] Fase de Varredura de Kernel-Mode e Detecção de Rootkits...")
        ...
        if rootkit_findings:
            ...
            print("[*] Detecções de rootkit salvas em kernel_rootkit_findings.json")
        else:
            print("[-] Nenhuma detecção de rootkit de kernel encontrada (via user-mode). ")

        # 3. Fase de Extração (Subtração) de Informações Suspeitas
        if os.path.exists("memory_scan_results.json"):
            print("\n[3] Fase de Extração (Subtração) de Memória Suspeita...")
            ...
            for entry in user_mode_findings:
                ...
                for detection in entry["detections"]:
                    if detection["type"] == "RWX_Private_Memory":
                        ...
                        size = detection["size"]
                        output_file = f"dumps/pid_{pid}_{name}_{detection["address"]}.bin"
                        print(f"[*] Realizando dump de {name} (PID: {pid}) no endereço {detection["address"]}...")
                        memory_dumper.dump_memory_region(pid, base_addr, size, output_file)
    else:
        print("\n[!] Ambiente não-Windows detectado. Algumas fases de kernel e extração foram ignoradas.")

    if all_findings:
        print("\n[+] RESUMO GERAL DAS DETECÇÕES:")
        ...
        print("\n[-] Nenhuma detecção geral encontrada.")

    print("\n" + "="*60)
    print(f"[*] Varredura concluída em: {time.ctime()}")
    print("="*60)
